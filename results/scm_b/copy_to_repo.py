...
root = "scm_bound"
destination = "/Users/alexandredrouin/dev/git/kover2_paper/models/scm_b"
...
for d in listdir(root):
    ...
    for sd in listdir(d):
        ...
        lines = lines[np.where([l.startswith("Model (")
                                for l in lines])[0][0]:]
        ...
        detailed_results_path = "../../../../../results/scm_b/{}/{}/repeat_{}/".format(quote(species), quote(antibiotic), seed)

        # open("{0!s}/model.txt".format(path), "w").write(model)
...
""".format(species[0].upper() + species[1:].lower(), antibiotic.title(), model, detailed_results_path)
