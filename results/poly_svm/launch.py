...
def generate_command(job_info, regularizer_values, degree_values, normalize_kernel_values, n_cpu):
    ...
    if not is_already_computed(split, KoverDataset(dataset_path).uuid, split_results_path):
        cmd = "python ./learner.py --dataset {0!s} " \
             ...
             "--regularizer {2!s} " \
             "--degree {3!s} " \
             "--normalize-kernel {4!s} " \
             "--ncpu {5:d} " \
             "--output {6!s}".format(dataset_path,
                                         split,
                                         ' '.join([str(x) for x in regularizer_values]),
                                         ' '.join([str(x) for x in degree_values]),
                                         ' '.join([str(x) for x in normalize_kernel_values]),
                                         n_cpu,
                                         split_results_path)
        return cmd
    return ""
...
""".format(walltime, i, "\n\n".join("(%s)&" % "; ".join(c) for c in grouped_commands))
...
if __name__ == "__main__":
    method_name = "kernel_svm_polynomial"
    ...
    regularizer_values = np.logspace(-7, 7, 15)
    degree_values = range(10)
    normalize_kernel_values = [0, 1]
    n_concurrent_hp = 8
    ...
    commands = np.array([generate_command(job_info,
                                          ...
                                          normalize_kernel_values=normalize_kernel_values,
